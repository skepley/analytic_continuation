    ...
    methods
        ...
        function productObj = mtimes(leftObj,rightObj,varargin)
            ...
            elseif isa(leftObj,'BAscalar') && isa(rightObj,'BAscalar') % Multiplication of 2 BAscalars (Cauchy product of analytic functions)
                if (leftObj.SurfaceDimension == 0 || rightObj.SurfaceDimension == 0)
        ...
        function invObj = inv(obj,varargin)
            % computes b = 1/obj by recursively solving the implicit
            % equation b*obj = 1
            
            switch ndims(obj.Modes)
                case 2
                    if obj.Modes(1) == 1 % 1d Taylor series
                        if nargin > 1
                            stopWhen = varargin{1}; % specify output size
                            A = BAscalar(obj.Coef,[1,stopWhen]);
                        else
                            stopWhen = obj.Modes(2); % output size matches obj size
                            A = BAscalar(obj.Coef);
                        end
                        initCond = 1/A.Coef(1); %scalar denominator
                        if initCond > 1e5
                            disp('Warning: constant is nearly zero. Invert function should not be trusted')
                        end
                        X(1)= initCond;
                        for k = 1:stopWhen-1
                            X(k+1) = -dot(A.Coef(2:k+1),flip(X))*initCond;
                        end
                    else % 2d Taylor series
                        if nargin > 1
                            error('2D inverter for specified size not yet implemented')
                        end
                        initRow = BAscalar(obj.Coef(1,:));
                        rowInv = initRow.inv; % row denominator
                        initCond = rowInv.Coef;
                        X = initCond;
                        for k = 1:obj.Modes(1)-1;
                            Xnum = -recursive_conv(obj.Coef(2:k+1,:),X(1:k,:),'row');
                            Xrow = recursive_conv(Xnum,initCond,'trunc');
                            X(k+1,:) = Xrow;
                        end
                    end
                case 3
