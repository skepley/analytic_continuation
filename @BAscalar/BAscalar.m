...
% Written by S. Kepley 06/2016 
% Changed SurfaceDimension property 06/2017  
% Rigorous subdivide method implemented 06/2017
% Added support for BAscalar coefficient type 07/2017 


%% ----------------------------------- TO DO -----------------------------------
% Move code out of class constructor 
% Vectorize methods
...
% migrate additional methods to class folder
    ...
    methods
        ...
        function intvalObj = intval(obj)
            ...
            if length(obj) > 1
                ...
                intvalObj.Coef(obj.Coef==0) = midrad(0,eps); % replace midrad(0,0) with midrad(0,eps) so that intLab will keep placeholder zeros
            end
        end
        ...
        function truncate(obj,modes)
            % truncate BAscalar to specified number of modes
            if ~isequal(obj.SurfaceDimension,2)
                error('not implemented for dimension other than 2 yet')
            end
            
            if isequal(length(obj),1) % a single BAscalar
                newModes = [min([obj.Modes(1),modes(1)]),min([obj.Modes(2),modes(2)])];
                obj.Coef = obj.Coef(1:newModes(1),1:newModes(2));
                obj.Coef = obj.Coef(1:newModes(1),1:newModes(2));
                obj.Coef = obj.Coef(1:newModes(1),1:newModes(2));
                obj.Coef = obj.Coef(1:newModes(1),1:newModes(2));
                obj.Modes = newModes;
        ...
        function evalObj = eval(obj,data)
            ...
            if length(obj) > 1 % vector of BAscalars
                ...
                switch obj.SurfaceDimension
                    ...
                    case 1 % data should be an m length column vector
                        s = reshape(data,1,[]);
                        S = bsxfun(@power,s,(0:obj.Modes-1)');
                        evalObj = obj.Coef*S;
